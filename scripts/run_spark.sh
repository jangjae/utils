...
exec_path=`pwd`
result_path=`pwd`/results/$sim_date1\.pot
...
# pagerank
# PR_input="twitter-2010.txt" # Nodes: 41,652,230, Edges: 1,468,364,884
# PR_input="facebook_combined.txt" # Nodes: 4,039, Edges: 88,234
# PR_input="com-lj.ungraph.txt" # [FIXME]
# PR_input="as-skitter.txt" # [FIXME]
PR_input="com-orkut.ungraph.txt" # Nodes: 3,072,441, edges: 117,185,083
...
set_java_env() {
  opt_comp=" -Xint"
  # opt_comp="-XComp -XX:-TieredCompilation"
...
do
  ...
  do
    ...
    # arranging dir
    if [ ! -d $result_path/${bm_prg[$i]}/$input.${bm_case[$j]} ]
    then 
      mkdir -p ${bm_prg[$i]}/$input.${bm_case[$j]}
    fi

    mv ${bm_prg[$i]}.* ${bm_prg[$i]}/$input.${bm_case[$j]}
    rm gcStatFromLog.py
    cd $exec_path
    # clean memchache

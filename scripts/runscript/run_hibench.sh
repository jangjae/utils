...
bm_prg=(pagerank terasort wordcount) 
...
exec_path=`pwd`
result_path=`pwd`/results/$sim_date1\.hibench.potential
script_path=`pwd`/../scripts
hadoop_path=$exec_path/../benchmarks/hadoop-1.2.1
spark_path=$exec_path/../benchmarks/spark-1.6.0
hibench_path=$exec_path/../benchmarks/HiBench
...
# hadoop & spark common -- should constent with spark conf
...
sed -i "9s@.*@export JAVA_HOME=$java_path@g" $hadoop_path/conf/hadoop-env.sh
...
run_hibench() {
  ...
  cp $hibench_path/conf/99-user_defined_properties.conf.jy $hibench_path/conf/99-user_defined_properties.conf
  sed -i "57s/.*/hibench.default.map.parallelism $spark_map_dop/g" $hibench_path/conf/99-user_defined_properties.conf
  sed -i "58s/.*/hibench.default.shuffle.parallelism $spark_shuffle_dop/g" $hibench_path/conf/99-user_defined_properties.conf
  sed -i "71s/.*/spark.executor.memory $spark_worker_memory/g" $hibench_path/conf/99-user_defined_properties.conf
  sed -i "72s/.*/spark.driver.memory $spark_driver_memory/g" $hibench_path/conf/99-user_defined_properties.conf
  sed -i "96s/.*/hibench.scale.profile $input/g" $hibench_path/conf/99-user_defined_properties.conf
  echo "" >> $hibench_path/conf/99-user_defined_properties.conf
  echo "spark.executor.extraJavaOptions $java_options" >> $hibench_path/conf/99-user_defined_properties.conf

  # prepare data
  if [ $prepare_flag == 0 ]
  then
    $hibench_path/workloads/${bm_prg[$j]}/prepare/prepare.sh
    prepare_flag=1
  fi

  (time $hibench_path/workloads/$bm/spark/scala/bin/run.sh) > $result_path/$bm.jvmlog 2>&1

  if [ -f $hibench_path/report/hibench.report ]
  then
    echo "${GREEN}$bm (scale: $input) is DONE"${NC}
  else
    echo "${RED}$bm (scale: $input) is error"${NC}
  fi

  mv $hibench_path/report/hibench.report $result_path/$bm.report
}
...
do
  ...
  do
    ...
    do
      ...
      pid_log="$!"
      nohup watch -t -n 1 "(numastat -m -z | sed -n '/^$numastat_event2/p' | awk '{ print $3 }') | tee -a $result_path/${bm_prg[$i]}.$numastat_event2.log" &
      pid_log2="$!"
      ...
      set_java_env $i
      # run_bench ${bm_prg[$i]}
      run_hibench ${bm_prg[$i]} 
      kill -9 $pid_log $pid_log2
      cd $exec_path
